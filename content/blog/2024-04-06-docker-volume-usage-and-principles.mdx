...
date: 2024-04-06T07:05:00.837Z
description: Docker 볼륨 (Volume) 사용 방법과 원리
meta:
  keywords:
    - docker
    - volume
...
1. **익명 볼륨 (Anonymous Volume)**
    ...
    - 호스트 측에는 Linux의 경우 `/var/lib/docker/volumes/` 아래에 공간이 확보되며, 컨테이너 내의 `/hoge` 디렉토리와 공유됩니다. 식별을 위해 해시 값이 할당됩니다. 동일한 네트워크에서 해당 해시 값을 사용하여 액세스할 수 있습니다.

2. **이름이 지정된 볼륨 (Named Volume)**
    - `$ docker container run -v name:/hoge sample/image:latest`
    - 익명 볼륨과 유사하게 호스트 측에는 Linux의 경우 `/var/lib/docker/volumes/` 아래에 공간이 확보되며, 컨테이너 내의 `/hoge` 디렉토리와 공유됩니다. `name`이라는 이름이 지정되어 있으므로 동일한 네트워크 내에서 `name` 호스트 이름으로 액세스할 수 있습니다.

3. **바인드 마운트 (Bind Mount)**
    - `$ docker container run -v ${PWD}/data:/hoge sample/image:latest`
    - 호스트 측의 현재 디렉토리 아래의 `data` 디렉토리와 컨테이너 측의 `/hoge` 디렉토리가 공유됩니다.
