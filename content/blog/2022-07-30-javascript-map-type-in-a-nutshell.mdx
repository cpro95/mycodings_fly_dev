...
자바스크립트 또는 타입스크립트의 Map Type에 대해 알아 보겠습니다.

Map 타입은 ES6에 도입되었는데요, key-value 라는 간단한 구조로 되어 있습니다.

객체나 어떤 primitive values라도 key 또는 value 값으로 들어 갈 수 있습니다.

자바스크립트 Map은 콜렉션이라고 부르는데요.

콜렉션이기 때문에 사이즈도 있고 순서도 있고, 각 아이템을 탐색할 수 도 있습니다.
...
위 코드를 보시면 타입스크립트로 명확하게 key-value에 해당되는 타입을 지정했습니다.

key는 string 타입이고 value는 number 타입입니다.

### 초기화 값을 지닌 Map 만들기

Map을 만들때 아래처럼 Map constructor에 배열로 key-value 값을 지정해 주면 초기화 값을 지닌 Map을 만들 수 있습니다.
...
4. map
  ...
  -- map entries를 iterating하기 위해 객체 디스트럭쳐링 사용
...
Map 타입을 필터링하려면 일단 Array.from() 메쏘드로 Map 타입을 배열로 가져와야 합니다.

아래처럼 Map 타입을 Array.from() 메쏘드로 배열로 전활 할 수 있는데요.
```js
const map1 = new Map([
  ['num1', 1],
  ['num2', 2],
...
그 다음으로 배열 디스트럭쳐링을 이해하야 합니다.

배열 디스트럭쳐링은 아래 코드를 보시면 쉽게 이해 할 수 있습니다.
...
map1의 value값이 1 초과되는 엔트리를 map1.delete 함수로 삭제하는 코드입니다.

iterating을 위해 forEach() method가 쓰였고요.

위와 같이 필터링 하는 방법도 있고 맵을 배열로 변환해서 배열의 filter 메쏘드를 사용하는 방법도 있습니다
...
객체를 맵 타입으로 변환하기 위해서는 객체가 맵 타입처럼 key-value 형식의 관계를 가지고 있으면 가능합니다.

Object.entries() 메쏘드를 쓰면 객체의 key-value 형식의 배열을 얻을 수 있는데 그걸 Map() 컨스트럭터에 넣으면 됩니다.

아래 코드처럼 Object.entries 메쏘드는 객체의 항목을 key-value 항목으로 변환해서 배열로 리턴합니다.
...
위 코드처럼 Object.entries() 메쏘드를 거친 배열의 첫번째 항목도 배열인데, 순차적으로 배열을 Map Constructor에 넣어서 Map을 생성하면 됩니다.
...
#### **반대로, Map 형식을 다시 Object 형식으로 변환시킬려면 어떻게 하면 될까요?**

여기서도 다시 Object.fromEntries() 메쏘드를 쓰면 됩니다.
...
Object.fromEntries() 메쏘드는 꼭 iterating 할 수 있는 Map 형식이나 배열을 받아야 하는데요.

Iterating 할 수 있는 key-value 형식이어야 하는 건 당연해야 합니다.

전체적인 코드를 보면 쉽게 이해 할 수 있을겁니다.
...
병합하고자 하는 Map 여러개를 스프레드 연산자(...)로 언팩하고 나서
...
Map 타입은 데이터가 삽인된 순서를 기억하고 있습니다.

그래서 배열로 변환되서도 순서가 그대로입니다.
...
여기까지는 Map의 Key 타입이 문자열(string)일때만 가능한데요.

문자열은 아스키코드로 정렬이 되기 때문에 개발자가 신경 안써도 되지만,

만약에 Map의 Key 타입이 숫자(number)일때는 sort() 함수를 만들어서 지정해 줘야 합니다.
...
틀린 점은 Map을 배열로 가져올때 key로 정렬할때는 배열의 첫번째 인자 즉, a[0], b[0]을 비교했지만,

value로 정렬할때는 배열의 두번째 인자인 a[1], b[1]을 비교하면 됩니다.
...
## Map에서 key, value 중 한개만 가져와서 배열 만들기

이 방법도 똑같이 Array.from() 메쏘드를 쓰면됩니다.
...
위 2개의 코드에서는 각각 map.keys() 메쏘드와 map.values() 메쏘드를 써서 Array.from()으로 전달했는데요.
...
Array.from() 메쏘드에 두번째 인자로 익명함수를 넣었습니다.

만약에 Array.from() 메쏘드에 두번째 인자를 안 넣고 그냥 첫번째 인자로 map을 넣는다면,

Array.from()은 아래 처럼 배열의 배열을 리턴할 겁니다.
...
그래서 Array.from() 메쏘드가 배열의 배열을 리턴한걸 두번째 인자인 익명 함수에서 처리하는 거죠.

두번째 인자인 익명 함수는 배열 디스트럭쳐링 방식이 쓰인 겁니다.
...
return {[key]: value}
```
형식을 써서 객체를 리턴하게 했습니다.

---

지금까지 자바스크립트의 Map 타입에 대해 알아 보았는데요.
