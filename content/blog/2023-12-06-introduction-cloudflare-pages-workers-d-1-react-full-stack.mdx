...
# 풀스택 강의 1편. Cloudflare Pages + Workers + D1 + React로 풀스택 개발하기
...
React의 JSX에 익숙해져 셔인지 결국에는 다시 React로 돌아오게 되더군요.
...
권한을 주면 위와 같이 터미널 상에서 본격적인 wrangler 명령어를 사용할 수 있게 됩니다.
...
그래서 제가 사용하는 방식은 Vite를 이용한 React 앱 구성후 Cloudflare 서비스를 연결하는 겁니다.

일단 1차적인 정적 사이트 호스팅이 완료되었네요.
...
보통 자기 계정이름과 함께 Workers 앱이 구현됩니다.

그래서 저 같은 경우 cpro95.workers.dev라는 서브 도메인이 있습니다.

그래서 workers를 새로 만들면 xxx.cprol95.workers.dev라는 주소가 되는 거죠.

물론 계정 이름 말고 다른 걸로 서브 도메인 이름을 정할 수 있습니다.

자기 만의 workers를 만들어 놓으면 자신만의 API를 구현할 수 있는 장점이 있고,
...
workers 함수는 위와 같이 onRequest 함수를 구현해야 하면 해당 함수는 Response를 리턴해야 합니다.
...
잠깐, 여기서 계속 Cloudflare 엣지 서버에 정식으로 배포(deploy) 하지 말고 개발 서버를 돌려서 확인할 수 있는 방법이 없을까요?
...
killall workerd
```
위와 같이 터미널 상에서 workerd 라는 worker demon을 모두 죽이면 됩니다.
...
그림으로 캡쳐를 안 했는데, 100% 제대로 작동하고 있는 걸 볼 수 있을 겁니다.
...
대시보드에서 직접 만들 수도 있고, 터미널 상에서 sql 파일을 이용해서 만들 수 도 있습니다.
