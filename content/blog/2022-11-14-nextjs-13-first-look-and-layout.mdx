...
date: 2022-11-14T11:14:28.634Z
description: Next.js 13 살펴보기 - 폴더 방식 라우팅과 중첩 레이아웃 이해
meta:
  keywords:
    - next.js
published: true
...
# Next.js 13 살펴보기 - 폴더 방식 라우팅과 중첩 레이아웃 이해
...
app 폴더 밑에는 무조건 한 개의 layout.tsx 파일이 있어야 되는데요.
...
위와 같이 RootLayout 컴포넌트가 있습니다. Next.js 13부터는 꼭 한 개의 RootLayout이 있어야 합니다.
...
기존 12 버전에서는 라우팅이 됐었는데요. 13 버전부터는 무조건 폴더 밑의 page.tsx 파일이 첫 번째로 읽히는 파일이 됩니다.
...
첫 번째 줄에서 TailwindCSS 관련 import를 했고,
...
이제 홈페이지를 만들었으니까 두 번째 파트로 넘어가 볼까요?
...
노란색 사각형이 바로 app/todos 폴더 밑에 있는 layout.tsx파일에 있는 겁니다.

사진을 보니까 이제 중첩된 레이아웃에 대해 이해할 수 있겠죠?

Todos 라우팅도 만들었으니까 여기에 데이터 fetching 하는 방법을 알아보겠습니다.

<hr />
## Next.js 13 버전부터 모든 React 컴포넌트는 서버사이드 컴포넌트입니다.

13 버전부터 Next.js가 의도하는 게 바로 기본 서버 사이드 컴포넌트인데요.

이게 뭘 의미하는지 앞으로 살펴볼 예정입니다.
...
위 코드를 보시면 TodosList 컴포넌트를 추가하고 있습니다.

{/* @ts-ignore */}라고 쓴 부분은 ES-Lint 에러인데요. 향후 Next.js 13 버전이 안정화되면 없애도 됩니다.

지금은 이걸 없애면 계속 TypeScript 경고가 떠서 귀찮기 때문입니다.

그럼 TodosList 컴포넌트를 만들어야겠네요.

/app/todos 폴더 밑에 컴포넌트를 만들어도 됩니다.

예전에는 별도 컴포넌트 폴더를 만들어서 그곳에 모두 저장했는데요.

Next.js 13부터는 해당 라우팅 폴더에 저장하면 됩니다. 뭔가 더 직관적이랄까요?
...
```
이렇게 타입스크립트 타입을 저장하면 타입스크립트가 알아서 타입을 찾아줍니다.
...
우리가 사용하는 todos API는 총 200개를 반환해 줍니다.
...
폴더 구조가 바로 '/app/todos/[todoId]' 이기 때문에 params는 todoId가 되고, 이 todoId에 string 값이 저장됩니다.
...
화면의 위쪽은 메뉴 리스트이고 옆 쪽도 해당 아이템 리스트일 거고 화면 중앙이 바로 해당 내용입니다.

Nested Layout을 적용하면 이렇게 세 가지의 분할된 로딩이 가능한데요.

화면 옆의 todo 번호를 누르면 화면 중앙의 상세페이지만 로도 되는 형식인데요.

기존 React에서는 전체 화면이 갱신되는 방식인데요.

Nested Layout 방식을 쓰면 필요한 부분만 다시 렌더링 되기 때문에 전체적인 페이지가 정말 빠르게 작동됩니다.
...
위 그림을 보시면 맨 위는 Header 컴포넌트이고 옆이 바로 Todo 넘버가 표시되는 부분이고 화면 중앙이 상세 페이지입니다.

위와 같이 만들려면 어떻게 해야 할까요?

일단 Header 컴포넌트는 안 건드는 거 같고, todos 폴더에  layout.tsx 파일을 만들어서 상세페이지를 todos 폴더에 있는 layout.tsx 파일에 삽입해야 합니다.

기존에는 todos 폴더 밑에 layout.tsx 파일을 안 만들었었는데요.

중첩 레이아웃을 구성하려면 /app/todos/layout.tsx 파일을 아래와 같이 만들어야 합니다.
...
todos/pages.tsx 파일의 내용을 위 layout.tsx에 옮기고 그리고 children을 추가했습니다.
...
todos 폴더 밑에 있는 layout.tsx파일에 있는 children props가 있는데요.

바로 todos 폴더 밑에 있는 '[todoId]' 라우팅에 children 자리에 들어가는 방식입니다.
...
이 Nested Layout 방식이 좋은 점은 바로 Todo 넘버링을 다시 로드되지 않고 순수하게 상세페이지만 로드된다는 점입니다.

<hr />
## Next.js 13 버전 처음 살펴본 소감

제가 Remix Framework에 심취해서 블로그도 새로 개편하고 여러 가지를 만들어 봤는데요.
...
다른 점은 Remix에서는 폴더 구조와 함께 파일도 같이 라우팅을 구성할 수 있다면 Next.js 13 버전은 오로지 폴더 라우팅 방식인 점이 틀립니다.

정말 코드를 작성하면서 Remix와 너무 닮았다고 느낀 게 많았는데요.

여러분들도 Remix를 꼭 한번 사용해 보시길 바랍니다.

다음 시간에는 Next.js 13 버전의 ISR, SSR 등에 대해 알아보겠습니다.
