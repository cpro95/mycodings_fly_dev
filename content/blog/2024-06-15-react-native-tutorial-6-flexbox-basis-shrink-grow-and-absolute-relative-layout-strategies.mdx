...
date: 2024-06-15T13:54:30.311Z
description: 이 강좌에서는 리액트 네이티브의 레이아웃 관리 방법을 자세히 설명합니다. 플렉스 기준(Flex Basis), 플렉스 축소(Flex Shrink), 플렉스 증가(Flex Grow)를 포함한 플렉스 박스의 주요 개념들을 배우고, 상대 레이아웃(Relative Layout)과 절대 레이아웃(Absolute Layout)의 차이점 및 적용 방법에 대해 알아봅니다.
meta:
  keywords:
    - react native
    - flexbox
